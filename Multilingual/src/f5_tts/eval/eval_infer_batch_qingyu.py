...
def main():
    ...
    parser.add_argument("-ns", "--expnamesp", required=True)
    ...
    parser.add_argument(
        "-p", "--librispeech_test_clean_path", default=f"<root_path>/LibriSpeech/test-clean", type=str
    )
    ...
    if testset == "ls_pc_test_clean":
        ...
        metalst = rel_path + "/data/test.lst"
        test_clean_path = "/vepfs/group09/F5-TTS_qingyu_MultilingualTTS/mytestset"
        metainfo = get_mytestset_metainfo(metalst, test_clean_path)
    ...
    output_dir = (
        ...
        f"_cfg{cfg_strength}_speed{speed}_{exp_name_sp}_{ckpt_step_sp}"
        f"{'_gt-dur' if use_truth_duration else ''}"
        f"{'_no-ref-audio' if no_ref_audio else ''}"
    )

    # -------------------------------------------------#
    qwen3_path = "/vepfs/group09/F5-TTS_qingyu_MultilingualTTS/qwen3_tokenizer_export"
    if tokenizer == "bpe":
    ...
    if mel_spec_type == "vocos":
        vocoder_local_path = "<root_path>/checkpoints/vocos-mel-24khz/"
