...
def deal_with_audio_dir(audio_dir):
    ...
    with open(audio_jsonl, "r") as f:
        ...
        for line in tqdm(lines, desc=f"{audio_jsonl.stem}"):
            ...
            lang_prefix = obj["wav"].split("_")[0]
            wav_name = os.path.splitext(obj["wav"])[0]
            rel_path = f"{lang_prefix}/{wav_name}"
            sub_result.append({"audio_path": str(audio_dir.parent / obj["wav"]), "text": text, "duration": duration, "rel_path": rel_path})
...
if __name__ == "__main__":
    ...
    save_dir = str(files("f5_tts").joinpath("../../")) + f"/data/{dataset_name}_gp"
