...
def number_to_hiragana(sentence):
    ...
    # Convert numbers to Japanese words (add spaces before and after)
    sentence = re.sub(r"\d+", lambda m: f" {Convert(m.group(), 'hiragana')} ", sentence)

    # Unify consecutive spaces into a single space
...
def transliterate(sentence, epi, eng_epi=None):
    # If English is found in the sentence, convert the English part separately with eng_epi
    # If no English is found, convert the Korean part with epi
    ...
    for ch in sentence:
        ...
        ):  # If it's an alphabet or space and not Japanese (special characters, etc.)
            if (
                current_eng_sentence or ch.isalpha()
            ):  # If English has already started or it's an alphabet (start of English). If current_sentence is not empty, it means English has started
                current_eng_sentence += ch
                if current_sentence:
                    ipa += epi.transliterate(current_sentence)
                    current_sentence = ""
            else:  # Otherwise (not in the middle of English - special character or space not in the middle of English)
                current_sentence += ch
            continue
